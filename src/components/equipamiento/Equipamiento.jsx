...
class Equipamiento extends React.Component {
    ...
    cargarDatos = () => {
        let url = urlApi + "equipamiento?page=" + this.state.pagina_actual + "&string=" + this.state.cadena_busqueda + "&limit=" + this.state.limite;
        axios
    ...
    eliminar = async (id, nombre) => {
        if (await confirm('¿Desea eliminar el equipamiento ' + nombre + '?')) {
            const { notificacion } = this.props;
            const url = urlApi + "equipamiento/" + id;
            axios
        ...
        return (
            <div>
                <div className="col-10 position-absolute top-0 start-50 translate-middle-x">
                    ...
                    <input type="text" onKeyPress={this.busqueda} className="form-control" placeholder="Busqueda por nombre del equipamiento y tipo" />
                    <table className="table">
                        ...
                        <tbody>
                            {this.state.registros.map((value, index) => {//Recorrer los registros
                                return (
                                    <tr key={index}>
                                        <th scope="row">{value.id_equipo}</th>
                                        <td>{value.nombre_equipo}</td>
                                        <td>{value.tipo}</td>
                                        <td>{value.estado}</td>
                                        <td>{value.cantidad}</td>
                                        <td>{value.fecha_adqui}</td>
                    ...
                    <div className="modal-overlay" style={modalStyles.overlay}>
                        <div className="modal-content" style={modalStyles.content}>
                            <FormularioEquipamiento
                                equipamientoAEditar = {this.state.equipo_seleccionado}
