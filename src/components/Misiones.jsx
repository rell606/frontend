...
import { notificacion } from "../../App";
import FormularioMisiones from "./FormularioMisiones";

class Misiones extends React.Component {
    ...
    mostrarModalNuevo = () => {
        this.setState({ mostrarModal: true, misionSeleccionada: null });
    }
    mostrarModalEditar = (id) => {
        this.setState({ mostrarModal: true, misionSeleccionada: id });
    }
    ...
    cargarDatos = () => {
        let url = urlApi + "misiones?page=" + this.state.pagina_actual + "&string=" + this.state.cadena_busqueda + "&limit=" + this.state.limite;
        axios
    ...
    busqueda = async (e) => {
        if (e.charCode === 13) { // Enter key
            this.setState({ cadena_busqueda: e.target.value, pagina_actual: 1 }, () => { this.cargarDatos() });
        }
    }

    eliminar = async (id, nombre) => {
        if (await confirm('¿Desea eliminar el registro ' + nombre + '?')) {
            const { notificacion } = this.props;
            const url = urlApi + "misiones/" + id;
            axios
        ...
        return (
            <div>
                <div className="col-10 position-absolute top-0 start-50 translate-middle-x">
                    ...
                    <br></br>
                    <br></br>
                    <input type="text" onKeyPress={this.busqueda} className="form-control" placeholder="Buscar por id, nombre o descripción" onChange={(e) => this.setState({ cadena_busqueda: e.target.value }, () => { this.cargarDatos() })} />
                    <br></br>
                    <br></br>
                    <table className="table">
                        <thead>
                            <tr>
                                <th scope="col">id_mision</th>
                                <th scope="col">nombre_mision</th>
                        ...
                        <tbody>
                            {this.state.registros.map((value, index) => {//Recorrer los registros
                                return (
                                    <tr key={index}>
                                        <th scope="row">{value.id_mision}</th>
                                        <td>{value.nombre_mision}</td>
                                        ...
                                        <td>
                                            ...
                                            <svg
                                                onClick={() => this.eliminar(value.id_mision, value.nombre_mision)}
                                                xmlns="http://www.w3.org/2000/svg"
                                                width="28"
                                                height="28"
                                                viewBox="0 0 24 24"
                ...
                {this.state.mostrarModal && (
                    <div className="modal-overlay" style={modalStyles.overlay}>
                        <div className="modal-content" style={modalStyles.content}>
                            <FormularioMisiones
                                misionAEditar={this.state.misionSeleccionada}
